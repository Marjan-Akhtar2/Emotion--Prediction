(() => {
  ...
  function renderResult(data, originalText) {
    // Extract base emotion (e.g., "sadness 😢" -> "sadness")
    const rawEmotion = (data.predicted_emotion || "").split(" ")[0].toLowerCase();
    const emotion = EMOJI[rawEmotion] ? rawEmotion : "joy";
    ...
    // Confidence float array or direct percentage value handling
    const confValue = data.confidence > 1 ? data.confidence : data.confidence * 100;
    el.confidenceText.textContent = `${confValue.toFixed(1)}% confidence`;
    el.echoedText.textContent = `“${originalText}”`;

    // FIX 1: Correct typo (all_probabilities instead of all_probabilites)
